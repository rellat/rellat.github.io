...
var identicon = ['./img/chase_thumb.jpg','./img/kenneth_thumb.jpg','./img/a01_thumb.png','./img/a02_thumb.png'];
// 댓글을 달때 사람사진 란에 들어갈 아이덴티콘을 담은 배열이다.
var personname = ['Chase Lee','Kenneth Lee','Lesser Panda','Dwarf Hamster'];
...
function getWidget(linehandle) {
  ...
  for (var j = 0; j < rellatreply.replies.length; ++j) {
    if(editor.getLineNumber(rellatreply.replies[j].widget.line) == no) {
      // line no가 일치하는 댓글노드의 line widget 오브젝트를 가져온다.
      return rellatreply.replies[j].widget;
    }
  }
  return undefined;
}
...
window.onload = function() {
  ...
  var text = document.getElementById('rellat_code01');
...
///
  var text2 = document.getElementById('rellat_doc01');
  var editor2 = DocEditable.fromTextArea(text2, {
    src: './rellat_storyboard',
    format: 'markdown'
  });

  var text3 = document.getElementById('rellat_terminal01');
  var editor3 = CodeMirror.fromTextArea(text3, {
    theme: "ambiance"
  });
...
// insert Git Graph
  var myTemplateConfig = {
    colors: [ "#0ad", "#a0d", "#0da", "#d0a" ], // branches colors, 1 per column
    branch: {
      lineWidth: 3,
      spacingX: 20,
  ...
  var master = gitGraph.branch({name:'master',column:3});
  master.commit('Initial commit');
  var develop = master.branch({name:'develop',column:2});
  // Add few commits on develop
  develop.commit({message:'create dev branch',tag:'1.0.0-SNAPSHOT'});
  develop.commit('quick little bit of work');
  // Merge "dev" branch into master with a tag
  develop.merge(master, {message:'release to prod', tag:'1.0.0'});
  develop.commit({message:'rev version',tag:'1.1.0-SNAPSHOT'});
  //
  // var fWIS3800 = develop.branch({name:'feature/WIS-3800',column:1});
  // fWIS3800.commit({message:'first attempt at WIS-3800',author:'Tyler'});
  // develop.commit('quick little bug fix');
  // var hDatatype = master.branch({name:'hotfix/bad-datatype',column:4});
  // hDatatype.commit({message:'need to fix the datatype',tag:'1.0.1-SNAPSHOT', author:'Forest'});
  // fWIS3800.commit({message:'working through some bugs',author:'Tyler'});
  // hDatatype.commit({message:'update a unit test', author:'Forest'});
  // hDatatype.merge(master, {message:'hotfix to prod PR#2', tag:'1.0.1', author:'Mark'});
  // master.merge(develop, {message:'bring in 1.0.1 hotfix bad datatype PR#3'});
  // fWIS3800.commit({message:'i think i got it working now',author:'Tyler'});
  // fWIS3800.merge(develop, {message:'WIS-3800 feature done PR#4'});
  // develop.merge(master, {message:'release to prod PR#5', tag:'1.1.0'});
  // develop.commit({message:'prep for next dev cycle',tag:'1.2.0-SNAPSHOT'});
